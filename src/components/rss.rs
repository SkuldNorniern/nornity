...
impl RSSGenerator {
    ...
    pub fn new(base_url: String, site_title: String, site_description: String, site_author: String) -> Self {
        Self {
            base_url,
            site_title,
            site_description,
    ...
    pub fn generate_rss_feed(&self, blog_store: &BlogStore) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        ...
        info!("Adding {} published blog posts to RSS feed", published_posts.len());
    ...
    fn generate_xml(&self, entries: &[RSSEntry]) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        ...
        xml.push_str(&format!("    <language>en-us</language>\n"));
        xml.push_str(&format!("    <lastBuildDate>{}</lastBuildDate>\n", self.format_rfc822(Utc::now())));
        xml.push_str(&format!("    <atom:link href=\"{}/rss.xml\" rel=\"self\" type=\"application/rss+xml\" />\n", self.base_url));
